            ...
            try
            {
                using (var conn = _dbService.GetConnection())
                {
                    ...
                    using (var cmd = new NpgsqlCommand(insertSql, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@MI", payload.MI ?? (object)DBNull.Value);
                        ...
                        cmd.Parameters.AddWithValue("@Password", payload.Password ?? "password123");
                        cmd.Parameters.AddWithValue("@ContactInfo", payload.ContactInfo ?? (object)DBNull.Value);
                        cmd.Parameters.AddWithValue("@ProfilePicture", payload.ProfilePicture ?? (object)DBNull.Value);
            ...
            try
            {
                using (var conn = _dbService.GetConnection())
                {
                    ...
                    using (var cmd = new NpgsqlCommand(updateSql, conn))
                    {
                        ...
                        if (hasNewPassword)
                        {
                            cmd.Parameters.AddWithValue("@Password", payload.Password ?? string.Empty);
                        }
