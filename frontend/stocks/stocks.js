...
// Converts 8-digit integer UTD → "YYYY/MM/DD"
...
// Converts "DateAdded" string (e.g. "2026-06-14 10:30:00") → "YYYY/MM/DD"
...
if (deleteConfirmBtn) {
    deleteConfirmBtn.addEventListener("click", async () => {
        const approver = deleteApprovalDropdown ? deleteApprovalDropdown.value : "";
        if (!approver) { alert("Please select an approver before confirming."); return; }

        const currentUser = getActiveSessionUser();
        if (!currentUser) return;

        const performedBy = formatUserIdentity(currentUser);
        deleteModal.classList.add("hidden");

        try {
            for (const batchId of _pendingDeleteBatchIds) {
                const res = await fetch(`${API_BASE_URL}/inventory/delete/${batchId}`, {
                    method: "DELETE",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ performedBy, approvedBy: approver })
                });
...
async function syncInventoryFromServer() {
    try {
        const response = await fetch(`${API_BASE_URL}/inventory?includeUnavailable=true`);
...
// ============================================================================
// DASHBOARD RECENT ACTIVITY MONITOR (TRUE HISTORY STREAM)
// ============================================================================
async function syncRecentActivityOnDashboard() {
    const activityContainer = document.getElementById("recentActivityContainer"); // 🎯 Match your HTML container ID
    if (!activityContainer) return;
 
    try {
        // Fetch from the real permanent ledger endpoint
        const response = await fetch(`${API_BASE_URL}/inventory/all-history`);
        if (!response.ok) throw new Error("Activity stream degradation.");
        ...
        if (recentLogs.length === 0) {
            activityContainer.innerHTML = '<p style="color: #777; font-style: italic; padding: 10px;">No recent operations logged.</p>';
            return;
        }
 
        // Render them dynamically into your dashboard widget layout
        activityContainer.innerHTML = recentLogs.map(log => {
            let badgeColor = "background: #d4edda; color: #28a745;"; // Stock-In / Add
            if (log.action.toLowerCase().includes("out")) {
                badgeColor = "background: #ffe0e3; color: #ff4757;"; // Stock-Out
            } else if (log.action.toLowerCase().includes("edit")) {
                badgeColor = "background: #fff3cd; color: #856404;"; // Edit Item
            }
            ...
            return `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px; border-bottom: 1px solid #eee; font-size: 13px;">
                    <div>
                        ...
                        <span style="padding: 2px 6px; border-radius: 4px; font-size: 10px; font-weight: bold; margin-left: 5px; ${badgeColor}">${log.action}</span>
                        <br>
                        <span style="font-size: 11px; color: #888;">Qty: ${log.quantity} | By: ${log.performedBy.split(',')[0]}</span>
                    </div>
        ...
        console.error("Failed to render dashboard activity feed:", error);
    }
}
